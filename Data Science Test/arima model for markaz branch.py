...
dataset_Marka = dataset[(dataset['Branch.Name.In.English'].str.rstrip()=='Marka') & (dataset['Activity.Type'].str.rstrip() == 'Branch Cash Withdrawal')]

# select only date and amount withdrawn column for arima
df = dataset_Marka.iloc[:,[0,6]]
...
df = df[df['Amount_withdrawn']<df['Amount_withdrawn'].quantile(0.996)]
...
'''both the rolling mean and rolling std dev are straight lines(parallel), hence
we can conclude that data is stationary.'''
...
'''ADF Test Statistic : -6.189418489199562
p-value : 6.174721361743384e-08
#Lags Used : 3
Number of Observations Used : 244
strong evidence against the null hypothesis(Ho), reject the null hypothesis. Data 
...
#plotting using arima model
from statsmodels.tsa.arima_model import ARIMA
model=ARIMA(df['Amount_withdrawn'],order=(4,0,0))
...
plt.title('RSS for ARIMA: %.4f'% sum((model_fit.fittedvalues-df['Amount_withdrawn'])**2))


#plotting using sarima model
import statsmodels.api as sm
model=sm.tsa.statespace.SARIMAX(df['Amount_withdrawn'],order=(4,0,0),seasonal_order=(4,0,0,5))
results=model.fit(display= -1)


plt.plot(df)
plt.plot(results.fittedvalues,color = 'red')
plt.title('RSS for SARIMA: %.4f'% sum((results.fittedvalues-df['Amount_withdrawn'])**2))

#predicting future values for arima
model_fit.plot_predict(1,270) #displays graph result
x_arima = model_fit.forecast(steps=22) #displays values in tuple format


#predicting future values for sarima
results.predict(start=246, end=270) #displays graph result
x_sarima = results.forecast(steps=22) #displays values in series format

...
model=ARIMA(df['Amount_withdrawn'],order=(1,0,0))
...
import statsmodels.api as sm
model=sm.tsa.statespace.SARIMAX(df['Amount_withdrawn'],order=(1,0,0),seasonal_order=(1,0,0,5))
...
x_arima = model_fit.forecast(steps=25) #displays values in tuple format


#predicting future values for sarima
results.predict(start=246, end=270) #displays graph result
x_sarima = results.forecast(steps=25) #displays values in series format
